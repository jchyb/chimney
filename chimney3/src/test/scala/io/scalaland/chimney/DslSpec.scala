...
class DslSpec extends AnyFreeSpec with Matchers {
  ...
  // utest bug workaround methods
  // keeping the utest methods signatures in scalatest
  extension [T] (inline t: T) {
    inline def ==>(inline s: T) = {
      t shouldBe s
    } 
  }
  inline def scalatestCompileError(inline code: String) = {
    assertTypeError(code)
  }
    ...
    "support different set of fields of source and target" - {
      ...
      "field is dropped - the target" in {
      ...
      "field is added to the target" - {
        ...
        "fill the field with provided default value" - {
          ...
          "not compile when selector is invalid" in { // TODO .transform replaces the expected error with "<notype> is illegal as a selection prefix
        ...
        "support default values for Options" - {
          ...
          "use None when .enableOptionDefaultsToNone" in {
            SomeFoo("foo").into[Foobar].enableOptionDefaultsToNone.transform ==> Foobar("foo", None)
          }

          "not compile if .enableOptionDefaultsToNone is missing" in {
          ...
          "target has default value, but default values are disabled and .enableOptionDefaultsToNone" in {
          ...
          "not compile if default value is missing and no .enableOptionDefaultsToNone" in {
            compileError("""SomeFoo("foo").into[Foobar].transform""")
              .check("", "Chimney can't derive transformation from SomeFoo to Foobar")
          }

          "not compile if default values are disabled and no .enableOptionDefaultsToNone" in {
        ...
        "use implicit transformer for option when .enableUnsafeOption" - {
          ...
          "use transformer when .enableUnsafeOption" in {
            Foobar(Some(1)).into[Foobar2].enableUnsafeOption.transform ==> Foobar2("1")
          }

          "use transformer when .disableUnsafeOption adn then .enableUnsafeOption" in {
    ...
    "support relabelling of fields" - {
      ...
      "not compile if relabelling selectors are invalid" in { // TODO .transform replaces the expected error with "<notype> is illegal as a selection prefix
      ...
      "not compile if relabelled - a wrong way" in {
    ...
    "support common data types" - {
      ...
      "support scala.Option" in {
        ...
        compileError("""Some("foobar").into[None.type].transform""")
          // .check("", "derivation from some: scala.Some to scala.None is not supported in Chimney!") // TODO will not work
      ...
      "support automatically filling of scala.Unit" in {
      ...
      "support Iterables collections" in {
      ...
      "support conversion between Iterables and Arrays" in {
      ...
      "support conversion between Iterables and Maps" in {
      ...
      "support conversion between Arrays and Maps" in {
    ...
    "support with .enableUnsafeOption" - {
      ...
      "preserve option to option mapping" in {
      ...
      "transforming None leads to NoSuchElementException" in {
        case class Foobar(x: Option[Int])
        case class Foobar2(x: String)
      ...
      "transforming fixed None type does not compile" in {
        compileError("""None.into[String].enableUnsafeOption.transform""")
          // .check("", "derivation from none: scala.None to java.lang.String is not supported in Chimney!") // TODO will not work
    ...
    "support using method calls to fill values from target type" - {
      ...
      "val and lazy vals work" in {
        assert(Foobar("param").into[Foobar2].transform == Foobar2("param", "valField", "lazyValField"))
      }

      "works with rename" in { // does not work in utest
        ...
        val res = Foobar("param")
          ...
          .transform
        assert(res == FooBar4(p = "param", v = "valField", lv = "lazyValField", m = "method1"))
      }

      "method is disabled by default" in {
        case class Foobar5(
            param: String,
            valField: String,
            lazyValField: String,
        ...
        compileError("""Foobar("param").into[Foobar5].transform""")
          .check(
            "",
      ...
      "protected and private methods are not considered (even if accessible)" - {
        ...
        compileError("""Foobar("param").into[Foo2].enableMethodAccessors.transform""")
        // .check(
        //   "",
        //   "protect: java.lang.String - no accessor named protect in source type io.scalaland.chimney.DslSpec.Foobar",
        //   "priv: java.lang.String - no accessor named priv in source type io.scalaland.chimney.DslSpec.Foobar" // TODO priv does not show
        // )
      }
    }
    ...
    "support sealed hierarchies" - {
      ...
      "enum types encoded as sealed hierarchies of case objects" - {
        ...
        "transforming flat and deep enum" in {
      ...
      "transforming flat and deep domains" in {
      ...
      "fail on ambiguous targets" in { // TODO error checks
        // using utest method results in an error:
        // children of trait Shape were already queried before class Triangle was discovered.
        // As a remedy, you could move class Triangle on the same nesting level as trait Shape.
        val error = scalatestCompileError(
    ...
    "support polymorphic source/target objects and modifiers" - {
      ...
      "automatically fill Unit parameters" in {
        case class Foo(value: String)
    ...
    "support abstracting over a value in dsl operations" in {
    ...
    "support scoped transformer configuration passed implicitly" - {
      ...
      "compile error when optionDefaultsToNone were disabled locally" in {
